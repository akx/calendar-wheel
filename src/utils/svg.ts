...
export function generateArcPathCommand(
  ...
  return `M${x1.toFixed(fractionDigits)},${y1.toFixed(fractionDigits)} A${radius} ${radius} 0 ${+largeArc} ${1 - +counterClockwise} ${x2.toFixed(fractionDigits)},${y2.toFixed(fractionDigits)}`;
}
...
export function generateFatArcPathCommand(
  ...
  return [
    `M${ix1.toFixed(fractionDigits)},${iy1.toFixed(fractionDigits)} A${innerRadius} ${innerRadius} 0 ${+largeArc} ${1 - +counterClockwise} ${ix2.toFixed(fractionDigits)},${iy2.toFixed(fractionDigits)}`,
    `L${ox2.toFixed(fractionDigits)},${oy2.toFixed(fractionDigits)}`,
    `A${outerRadius} ${outerRadius} 0 ${+largeArc} ${+counterClockwise} ${ox1.toFixed(fractionDigits)},${oy1.toFixed(fractionDigits)}`,
    `Z`,
  ].join(" ");
}
